...
namespace Dialer_AutoProgressive.Business
{
    public class ClsLeadCollection : ClsBaseCollection<ClsLead>
    {
        public static ClsLeadCollection GetAll(long userid, out long campaingID, ClsUserDataService clsDataService)//,int totalRecordToFetch)
        {
            ...
            {
                VMuktiAPI.VMuktiHelper.ExceptionHandler(ex, "ClsLeadCollection()", "ClsLeadCollection.cs");
                //System.Windows.Forms.MessageBox.Show("Cant Connect to Database " + ex.Message);
                campaingID = 0;
                return null;
            }
        }
