...
namespace Group.Business
{
    public class ClsUserCollection : ClsBaseCollection<ClsUser>
    {
        ...
        //    return sb;
        //}
        public static ClsUserCollection GetAll(int GroupId)
        {
            try
            {
                ClsUserCollection obj = new ClsUserCollection();
                //Calling User_GetAll for getting all available users
                obj.MapObjects(new ClsUserDataService().User_GetAll(GroupId));
                return obj;
            }
            catch (Exception ex)
            {
