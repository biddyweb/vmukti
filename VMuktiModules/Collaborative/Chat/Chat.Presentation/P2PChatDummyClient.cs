...
{
    [Serializable]
    public class P2PChatDummyClient
    {
        ...
        public P2PChatDummyClient(string uname)
		{
			try
			{
				this.UserName = uname;
			...
			{
                VMuktiAPI.VMuktiHelper.ExceptionHandler(ex, "P2PChatDummyClient", "P2PChatDummyClient.cs");
			}
        }

        public void ChatP2PClient(string ID,string P2PUri)
		{
			try
			{              
                AppDomainSetup setup = new AppDomainSetup();
                setup.ApplicationBase = AppDomain.CurrentDomain.BaseDirectory ;
                appDummyDomains.Add(AppDomain.CreateDomain("Chatp2pClient" + ID.ToString(), null, setup, new System.Security.PermissionSet(PermissionState.Unrestricted)));
                ...
                objChatDummies.Add(InstantiateDecimal(appDummyDomains[appDummyDomains.Count - 1], new DomainBinder(), new CultureInfo("en-US"), UserName,P2PUri));

			}
			catch (Exception ex)
			{
                VMuktiAPI.VMuktiHelper.ExceptionHandler(ex, "ChatP2PClient", "P2PChatDummyClient.cs");
			}
        }
        ...
        static object InstantiateDecimal(AppDomain appDomain, Binder binder, CultureInfo cultureInfo,string UName, string P2PUri)
        {
            try
            {
                object instance = appDomain.CreateInstance(
                   "Chat.Presentation",
                   "Chat.Presentation.P2PChatClient",
                   false,
            ...
            {
                VMuktiAPI.VMuktiHelper.ExceptionHandler(ex, "InstantiateDecimal", "P2PChatDummyClient.cs");
                return null;
            }
        }   
