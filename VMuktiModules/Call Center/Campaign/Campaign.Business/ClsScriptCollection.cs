    ...
    public class ClsScriptCollection : ClsBaseCollection<ClsScript>
    {
        ...
        public static ClsScriptCollection GetAll()
        {
            try
            {
                ...
                //Function for getting all script data from ClsCampaignDataService
                obj.MapObjects(new ClsCampaignDataService().Script_GetAll());
                return obj;
            }
            catch (Exception ex)
            {
                VMuktiAPI.VMuktiHelper.ExceptionHandler(ex, "GetAll()", "ClsScriptCollection.cs");
            }
            return null;
        }
    }
